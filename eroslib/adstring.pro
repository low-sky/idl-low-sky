...
;-
 ...
 if not keyword_set(truncate) then begin
    ...
    if Nzero GT 0 then leadzero[less10] = ' 0'
     result = string(ihr,'(I3.2)') + delimiter + string(imin,'(I2.2)') + $
              delimiter + strcompress(leadzero + $
                                      strtrim(string(xsec, secfmt), 2), /rem)+ '  ' 
;    if (Npar LT 3) then precision = 1
  ...
  endelse
   ...
   return, result + deg + delimiter+ string(imn,'(I2.2)') + delimiter + $
     strcompress(leadzero + strtrim(string(xsc, secfmt), 2), /rem)
