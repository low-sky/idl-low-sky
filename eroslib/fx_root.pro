;$Id: fx_root.pro,v 1.1 2003/07/14 17:47:24 eros Exp $
;
...
;
;       Tue Jun 17 10:43:46 2003, <eros@master>
;		Hacked to include extra keywords to function.
; Written by: GGS, RSI,
;March 1994 Modified: GGS, RSI, September 1994 Added support for
;double-precision complex inputs.  -

function fx_root, xi, func, double = double, itmax = itmax, $
                            stop = stop, tol = tol, _ref_extra = ex
  ...
  while (it lt itmax and cond ne 1) do begin
    q = (x[2] - x[1])/(x[1] - x[0])
    pls = (1 + q)
    f = call_function(func, x, _extra = ex)
    ...
    if stop eq 0 and abs(root - x[2]) le tol then begin
        cond = 1
            ...
            evalFunc = call_function( func, root, _extra = ex)
