pro clickplot2, cube, cube2, nomask = nomask, mask = mask, $
...
if keyword_set(nomask) then disp, total(cube, 3) else begin
  if keyword_set(mask) then disp, total(cube*mask, 3) else cubemap, cube
endelse
if not keyword_set(multiplier) then multiplier = 1
!mouse.button= 0
...
;     strcompress(string(y), /rem)+')'
   oplot, multiplier*cube2[x, y, *], psym = 10, color = !red
   break:
   wset,0
